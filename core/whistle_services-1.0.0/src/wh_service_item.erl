...
-record(wh_service_item, {category :: api_binary()
                          ...
                          ,rate :: api_binary()
                          ,single_discount = 'false' :: api_boolean()
...
%% @end
%%--------------------------------------------------------------------
-spec rate(item()) -> api_binary().
rate(#wh_service_item{rate='undefined'}) ->
    'undefined';
rate(#wh_service_item{rate=Rate}) ->
    wh_util:to_binary(Rate).
...
set_rate(Rate, #wh_service_item{}=ServiceItem) ->
    ServiceItem#wh_service_item{rate=wh_util:to_binary(Rate)}.
...
%% @end
%%--------------------------------------------------------------------
-spec single_discount(item()) -> api_boolean().
single_discount(#wh_service_item{single_discount='undefined'}) ->
    'undefined';
single_discount(#wh_service_item{single_discount=SingleDiscount}) ->
    wh_util:is_true(SingleDiscount).
...
%% @end
%%--------------------------------------------------------------------
-spec set_single_discount(api_boolean(), item()) -> item().
set_single_discount('undefined', #wh_service_item{}=ServiceItem) ->
    ServiceItem#wh_service_item{single_discount='undefined'};
...
single_discount_rate(#wh_service_item{single_discount_rate='undefined'}) ->
    'undefined';
...
%% @end
%%--------------------------------------------------------------------
-spec set_single_discount_rate(api_float(), item()) -> item().
...
cumulative_discount(#wh_service_item{cumulative_discount='undefined'}) ->
    'undefined';
cumulative_discount(#wh_service_item{cumulative_discount=Quantity}) ->
    wh_util:to_integer(Quantity).
...
%% @end
%%--------------------------------------------------------------------
-spec set_cumulative_discount('undefined' | integer(), item()) -> item().
...
cumulative_discount_rate(#wh_service_item{cumulative_discount_rate='undefined'}) ->
    'undefined';
...
%% @end
%%--------------------------------------------------------------------
-spec bookkeeper(wh_json:key(), item()) -> wh_json:json_term().
